"""Complete, capture-backed Cromwell 92 workflow-abort behavior contract."""
...
WDL_PATH = CASE_ROOT / "abort.wdl"
INPUTS_PATH = CASE_ROOT / "abort.inputs.json"
CAPTURE_PATH = CASE_ROOT / "capture.json"
CASE_METADATA_PATH = CASE_ROOT / "case_metadata.json"

PROGRAM_ID = "cromwell_workflow_abort_v1"
SUBJECT_ID = "cromwell_workflow_submission_service"
PORT = 59639
ORIGIN = f"http://127.0.0.1:{PORT}"
DISPOSABLE_ROOT = Path("/tmp/datalox-cromwell-92-workflow-abort-v1")
EXECUTION_ROOT = DISPOSABLE_ROOT / "executions"
OWNERSHIP_MARKER = b"datalox-cromwell-92-workflow-abort-v1\n"
ABORT_BOUNDARY = "DataloxCromwellAbortBoundary"
UNKNOWN_WORKFLOW_ID = "00000000-0000-0000-0000-000000000000"
...
def _abort_multipart() -> v3.MultipartFormDataSpec:
...
def _running_poll_spec() -> v3.PollSpec:
    return v3.PollSpec(
        ...
        allowed_intermediate_values=("Submitted",),
        terminal_values=("Running", "Aborting", "Succeeded", "Failed", "Aborted"),
        accepted_terminal_values=("Running",),
    )


def _aborted_poll_spec() -> v3.PollSpec:
    return v3.PollSpec(
        interval_ms=POLL_INTERVAL_MS,
        max_attempts=POLL_MAX_ATTEMPTS,
        deadline_ms=POLL_DEADLINE_MS,
        ...
        status_pointer="/status",
        allowed_intermediate_values=("Running", "Aborting"),
        terminal_values=("Submitted", "Succeeded", "Failed", "Aborted"),
        accepted_terminal_values=("Aborted",),
    )
...
def build_recipe() -> v3.BehaviorRecipe:
    ...
    execution_prefix = f"{EXECUTION_ROOT}/abort_case/"
    ...
    abort_multipart = _abort_multipart()
    abort_request = _request("POST", (*primary_path, "/abort"))
    return v3.BehaviorRecipe(
        ...
        steps=(
            v3.BehaviorStep(
                ...
                request=_request("POST", "/api/workflows/v1", body=abort_multipart),
            ...
            v3.BehaviorStep(
                ...
                assertions=(
                    _status("running_poll_status", 200),
                    _json_equals("running_poll_exact", "/status", "Running"),
                ),
                poll=_running_poll_spec(),
            ),
            v3.BehaviorStep(
                step_id="abort_primary",
                operation_id="cromwell.workflows.abort",
                kind="mutation",
                role="success",
                expected_outcome="mutation_success",
                subject_id=SUBJECT_ID,
                ...
                assertions=(
                    _status("abort_status", 200),
                    _json_equals("abort_aborting", "/status", "Aborting"),
                ),
            ),
            v3.BehaviorStep(
                step_id="abort_primary_duplicate",
                operation_id="cromwell.workflows.abort",
                kind="mutation",
                role="duplicate",
                ...
                assertions=(
                    v3.AssertionSpec(
                        ...
                        prior_step_id="abort_primary",
                    ),
                    _json_equals(
                        "duplicate_abort_aborting",
                        "/status",
                        "Aborting",
                    ),
                ),
            ...
            v3.BehaviorStep(
                step_id="abort_unknown_workflow",
                operation_id="cromwell.workflows.abort",
                kind="mutation",
                role="native_failure",
                ...
                request=_request(
                    "POST",
                    f"/api/workflows/v1/{UNKNOWN_WORKFLOW_ID}/abort",
                ),
                assertions=(
                    _status("unknown_abort_status", 404),
                    _json_equals(
                        "unknown_abort_exact_body",
                        "",
                        {
                            "status": "error",
                            "message": (
                                f"Couldn't abort {UNKNOWN_WORKFLOW_ID} because no "
                                "workflow with that ID is in progress"
                            ),
                        },
                    ),
                ),
            ...
            v3.BehaviorStep(
                ...
                request=_request("GET", (*primary_path, "/status")),
                assertions=(
                    _status("aborted_poll_status", 200),
                    _json_equals("aborted_poll_exact", "/status", "Aborted"),
                ),
                poll=_aborted_poll_spec(),
            ),
            v3.BehaviorStep(
                ...
                assertions=(
                    ...
                    _json_equals("outputs_empty", "/outputs", {}),
                ),
            ),
            v3.BehaviorStep(
            ...
            v3.BehaviorStep(
                ...
                assertions=(
                    _json_equals("metadata_aborted", "/status", "Aborted"),
                    _json_equals("metadata_outputs_empty", "/outputs", {}),
                    _json_equals(
                        "metadata_backend_local",
                        "/calls/abort_case.wait_long_enough_to_abort/0/backend",
                        "Local",
                    ),
                    _json_equals(
                        "metadata_execution_aborted",
                        (
                            "/calls/abort_case.wait_long_enough_to_abort/0/"
                            "executionStatus"
                        ),
                        "Aborted",
                    ),
                    v3.AssertionSpec(
                        assertion_id="running_to_aborted_observed",
                        kind="state_observe_step",
                        pointer="/status",
                        prior_step_id="poll_primary_running",
...
def build_connector(fixture_receipt: dict[str, Any]) -> v3.ConnectorSpec:
    ...
    return v3.ConnectorSpec(
        connector_id="cromwell_92_local_workflow_abort_v1",
...
class CromwellAbortTargetError(RuntimeError):
    """The caller diverged from the exact captured Cromwell abort program."""


class CromwellAbortBehaviorTarget:
    """Exact captured-program projection; no broader Cromwell semantics are claimed."""

    target_id = "cromwell_abort_captured_program_projection_v1"
    target_version = "workflow_abort_v1"
    ...
    def __init__(
        ...
        *,
        ...
        self._terminal_exchanges = tuple(
            grouped[step["step_id"]][-1] for step in self._steps
        )
        self._index = 0
    ...
    def _validated_uuid(value: str, *, name: str) -> str:
        if type(value) is not str:
            raise CromwellAbortTargetError(f"{name} must be a UUID string")
        try:
            parsed = uuid.UUID(value)
        except ValueError as error:
            raise CromwellAbortTargetError(f"{name} must be a UUID string") from error
        if str(parsed) != value:
            raise CromwellAbortTargetError(f"{name} must use canonical UUID syntax")
        return value

    def reset(self, seed: int) -> None:
        if seed != self._seed:
            raise CromwellAbortTargetError("seed does not match the captured program")
        self._index = 0

    def execute(self, call: ReferenceCall) -> ObservedResponse:
        if self._index >= len(self._steps):
            raise CromwellAbortTargetError("captured abort program is already finished")
        ...
        if actual != expected:
            raise CromwellAbortTargetError(
