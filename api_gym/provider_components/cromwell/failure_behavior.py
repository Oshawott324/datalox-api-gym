"""Complete, capture-backed Cromwell 92 workflow-failure behavior contract."""
...
WDL_PATH = CASE_ROOT / "failure.wdl"
INPUTS_PATH = CASE_ROOT / "failure.inputs.json"
CAPTURE_PATH = CASE_ROOT / "capture.json"
CASE_METADATA_PATH = CASE_ROOT / "case_metadata.json"

PROGRAM_ID = "cromwell_workflow_failure_v1"
SUBJECT_ID = "cromwell_workflow_submission_service"
PORT = 59638
ORIGIN = f"http://127.0.0.1:{PORT}"
DISPOSABLE_ROOT = Path("/tmp/datalox-cromwell-92-workflow-failure-v1")
EXECUTION_ROOT = DISPOSABLE_ROOT / "executions"
OWNERSHIP_MARKER = b"datalox-cromwell-92-workflow-failure-v1\n"
FAILURE_BOUNDARY = "DataloxCromwellFailureBoundary"
...
def _failure_multipart() -> v3.MultipartFormDataSpec:
    return v3.MultipartFormDataSpec(
        ...
        parts=(
            v3.MultipartPartSpec(
                ...
                filename="failure.wdl",
                media_type="application/octet-stream",
            ),
            v3.MultipartPartSpec(
                name="workflowInputs",
                artifact_ref="inputs",
                filename="failure.inputs.json",
...
def _poll_spec() -> v3.PollSpec:
    return v3.PollSpec(
        ...
        accepted_terminal_values=("Failed",),
    )
...
def build_recipe() -> v3.BehaviorRecipe:
    ...
    execution_prefix = f"{EXECUTION_ROOT}/failure_case/"
    call_prefix = "/call-exit_nonzero"
    primary_binding = v3.BindingSpec(
        ...
        composed_string_occurrences=(
            ...
            _composed(
                ...
                "/message",
                prefix="Couldn't abort ",
                suffix=" because no workflow with that ID is in progress",
            ),
            _composed(
                "primary_metadata",
    ...
    failure_multipart = _failure_multipart()
    poll = _poll_spec()
    return v3.BehaviorRecipe(
        ...
        steps=(
            ...
            v3.BehaviorStep(
                ...
                subject_id=SUBJECT_ID,
                auth_context_id="loopback_anonymous",
                request=_request("POST", "/api/workflows/v1", body=failure_multipart),
            ...
            v3.BehaviorStep(
                ...
                assertions=(
                    ...
                    _json_equals("primary_poll_failed", "/status", "Failed"),
                ),
                poll=poll,
            ),
            v3.BehaviorStep(
                ...
                assertions=(
                    ...
                    _json_equals("duplicate_poll_failed", "/status", "Failed"),
                ),
                poll=poll,
            ),
            v3.BehaviorStep(
            ...
            v3.BehaviorStep(
                step_id="abort_terminal_failed_primary",
                ...
                subject_id=SUBJECT_ID,
                auth_context_id="loopback_anonymous",
                request=_request("POST", (*primary_path, "/abort")),
                assertions=(
                    _status("terminal_abort_status", 404),
                    _json_equals("terminal_abort_error", "/status", "error"),
                    v3.AssertionSpec(
                        assertion_id="terminal_abort_message",
                        kind="json_pointer_pattern",
                        pointer="/message",
                        pattern=(
                            "Couldn't abort [0-9a-f]{8}-[0-9a-f]{4}-"
                            "[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12} because "
                            "no workflow with that ID is in progress"
                        ),
                    ),
                ),
            ),
            v3.BehaviorStep(
                ...
                assertions=(
                    _json_equals("metadata_failed", "/status", "Failed"),
                    _json_equals(
                        "metadata_backend_local",
                        "/calls/failure_case.exit_nonzero/0/backend",
                        "Local",
                    ),
                    _json_equals(
                        "metadata_execution_failed",
                        "/calls/failure_case.exit_nonzero/0/executionStatus",
                        "Failed",
                    ),
                    _json_equals(
                        "metadata_return_code",
                        "/calls/failure_case.exit_nonzero/0/returnCode",
                        23,
                    ),
                    _json_equals(
                        "metadata_not_retryable",
                        "/calls/failure_case.exit_nonzero/0/retryableFailure",
                        False,
                    ),
                    v3.AssertionSpec(
                        assertion_id="submitted_to_failed_observed",
...
def build_connector(fixture_receipt: dict[str, Any]) -> v3.ConnectorSpec:
    ...
    return v3.ConnectorSpec(
        connector_id="cromwell_92_local_workflow_failure_v1",
...
class CromwellFailureTargetError(RuntimeError):
    """The caller diverged from the exact captured Cromwell failure program."""


class CromwellFailureBehaviorTarget:
    """Exact captured-program projection; no broader Cromwell semantics are claimed."""

    target_id = "cromwell_failure_captured_program_projection_v1"
    target_version = "workflow_failure_v1"
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise CromwellFailureTargetError("generated workflow IDs must be distinct")
    ...
    def _validated_uuid(value: str, *, name: str) -> str:
        if type(value) is not str:
            raise CromwellFailureTargetError(f"{name} must be a UUID string")
        try:
            parsed = uuid.UUID(value)
        except ValueError as error:
            raise CromwellFailureTargetError(f"{name} must be a UUID string") from error
        if str(parsed) != value:
            raise CromwellFailureTargetError(
                f"{name} must use canonical UUID syntax"
            )
        return value

    def reset(self, seed: int) -> None:
        if seed != self._seed:
            raise CromwellFailureTargetError("seed does not match the captured program")
        self._index = 0

    def execute(self, call: ReferenceCall) -> ObservedResponse:
        if self._index >= len(self._steps):
            raise CromwellFailureTargetError(
                "captured failure program is already finished"
            )
