#!/usr/bin/env python3
"""Capture the complete Cromwell 92 ABORTED behavior through generic V3."""
    ...
    *,
    ...
    with disposable_cromwell_fixture(
        ...
        result = v3.BehaviorHarvester().run(
            ...
            expected_engine=ENGINE_IDENTITY,
            run_id="cromwell_workflow_abort_20260730",
        ...
        running_polls = [
            ...
            if item.step_id == "poll_primary_running"
        ]
        aborted_polls = [
            item
            for item in capture.exchanges
            if item.step_id == "poll_primary_aborted"
        ]
        transient_404 = sum(item.status_code == 404 for item in running_polls)
        metadata = {
            ...
            "coverage": {
                ...
                "running_poll_calls": len(running_polls),
                "aborted_poll_calls": len(aborted_polls),
            ...
            "claims": {
                "successful_abort": (
                    "provider_native_http_200_aborting_from_running"
                ),
                "duplicate_abort": (
                    "byte_identical_repeat_observed_http_200_aborting"
                ),
                "native_failure": (
                    "provider_native_http_404_abort_unknown_all_zero_uuid"
                ),
                "fixture": (
                    "disposable_loopback_cromwell_92_local_hsqldb_file"
                ),
                "resulting_state_relation": (
                    "observed_running_to_aborted_changed"
                ),
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Capture one complete Cromwell 92 ABORTED behavior program."
    )
